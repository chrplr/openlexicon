...
helper_alert <-
  tags$div(id = "helper_box",
           ...
           tags$ul(
             ...
             tags$li("The", lenGram_choice, "option is set automatically, depending on the", length_choice, "option. More details", tippy(tags$span(style="color:red; text-decoration:underline; font-weight:bold","here."), paste("The generation of pseudowords of length >= 5 is based on the random selection of an initial trigram (the", tags$b("trigram"), "algorithm). Then, compatible trigrams are selected until the generation of the pseudoword is complete. For shorter pseudowords (length < 5), we obtain better performance when replacing trigrams with bigrams (the", tags$b("bigram"), "algorithm). If you want to try both algorithms with different pseudoword lengths, you can change the", lenGram_choice, "option."), theme="light-border", placement="right")),
             tags$li('Then, paste a custom list with words of the desired pseudowords length in the', tags$b(paste_words), 'section. Alternatively, you can click the', btn_show_generator_name, 'button, and generate words from an', tags$b("OpenLexicon"), 'database :'),
               tags$ul(
