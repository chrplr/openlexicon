...
hamming_tooltip = paste("<span",tooltip_style,">The <b>Hamming distance</b> option computes the Hamming distance (the number of non-overlapping characters) between words of the same length. The computation is done between each item and the Lexique 3 database. For each source, we display the number of neighbors with distance 1. For more information : <a href = 'https://www.rdocumentation.org/packages/vwr/versions/0.3.0/topics/hamming.distance'>hamming.distance</a>. <b>Please note that selecting this option will significantly increase computation time.</b></span>")
